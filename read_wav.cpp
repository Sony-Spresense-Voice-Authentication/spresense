...
WavFileReader::WavFileReader() : dataSize(0), audioFormat(0), numChannels(0), sampleRate(0), bitsPerSample(0) {}
...
size_t WavFileReader::readSamples(int16_t* buffer, size_t bufferSize) {
    if (!file) {
...
bool WavFileReader::parseHeader() {
    if (!file) {
        return false;
    }
